...
## Hotfix M2.4.3.0.1 (2026-05-06 post test run)

**Bug**: `test_internal_link_endpoints_share_flow_id` fallito con `'manuf-out-uuid' == '818e9514-...'`.

**Root cause**: provider output exchange (`FlowRefDataset`) usava il `ref.uuid` ecoinvent come Flow @id, mentre il consumer input (`FlowRefInternal`) calcolava già il `_internal_flow_id()` UUIDv5 deterministico — i due endpoint quindi divergevano e openLCA non riusciva a stitchare il `processLink` su import.

**Fix**: aggiunta helper `_collect_internal_link_targets(processes) -> set[(provider_id, output_exchange_id)]` invocata in cima a `build_zolca_process_based`. In `_build_one_process`, quando un'output exchange `(pb_process.id, ex.id)` ricade nel set dei target referenziati come `internal_process`, la sua `flow.id` viene sovrascritta con `_internal_flow_id(project_id, pb_process.id, ex.id)` — la stessa funzione già usata dal lato consumer. Provider e consumer ora producono identicamente lo stesso UUIDv5 deterministico.

**Files**: `backend/services/zolca_builder_process_based.py` (+30 righe: helper + parametro keyword `internal_link_targets` + branch `elif` di override). Nessun nuovo test, nessun rifattoring oltre il minimo, flat mode invariato.

**Verifica**: 29/29 test M2.4.3 pass (`test_zolca_process_based.py` 11/11 + `test_suggest_flow_type.py` 9/9 + `test_extend_index_elementary.py` 10/10 — il file process-based contiene 11 test, lo spec citava 9 nel sottoinsieme rilevante). Suite completa pre-fix: 41 failed, post-fix: 40 failed → unica differenza è il test target che ora passa; gli altri 40 sono pre-esistenti (fixture/env issues fuori scope hotfix). Round-trip openLCA Desktop e ChromaDB extraction restano carry-over post-merge come da report originale.

**Commit**: `3e21c7a` su `claude/chromadb-zolca-rewrite-Yh8YD` (force-push, PR #16 aggiornata, NO nuovo PR).
